...
echo "TurboACC: 25.12 APK 仓库暂未提供兼容包，自动跳过。"

if [ -n "$CUSTOM_PACKAGES" ]; then
  git clone --depth=1 https://github.com/wukongdaily/apk.git /tmp/mzwrt-apk
  mkdir -p extra-packages
  cp -r /tmp/mzwrt-apk/run/x86/* extra-packages/
...
OPENCLASH_URL=$(curl -fsSL https://api.github.com/repos/vernesong/OpenClash/releases/latest | grep 'browser_download_url.*apk' | head -n1 | cut -d '"' -f 4)
