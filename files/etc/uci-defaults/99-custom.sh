...
# --- 1. 设置固件描述 (作者信息) 和设备名称 (Hostname) ---
# 设置编译作者信息
FILE_PATH="/etc/openwrt_release"
NEW_DESCRIPTION="Packaged by wukongdaily" # 保持您最新的作者信息
sed -i "s/DISTRIB_DESCRIPTION='[^']*'/DISTRIB_DESCRIPTION='$NEW_DESCRIPTION'/" "$FILE_PATH"
echo "已设置固件描述为: $NEW_DESCRIPTION" >> $LOGFILE

# 设置设备名称 (Hostname) 为 Mz.WRT
NEW_HOSTNAME="Mz.WRT"
...
echo "已设置防火墙 zone[1] input 为 ACCEPT" >>$LOGFILE
...
        # 检查是否为物理接口并以 eth 或 en 开头
        ifnames="$ifnames $iface_name"
    fi
done
ifnames=$(echo "$ifnames" | awk '{$1=$1};1') # 去除多余空格并修剪
...
    echo "已处理 luci-app-advancedplus 的 zsh 调用问题。" >>$LOGFILE
fi

# 统一提交剩余的 UCI 更改
uci commit

echo "Finishing 99-custom.sh at $(date)" >>$LOGFILE
exit 0
